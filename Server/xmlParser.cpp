...
char *_tcscpy_s(char *c1, const char *c2) { return (char*)strcpy(c1,c2); }
#endif
...
LPTSTR toXMLString(LPTSTR dest,LPCTSTR source)
{
    ...
    while (*source)
    {
        switch (*source)
        {
        case '<' : _tcscpy_s(dest,sizeof(dest),_T("&lt;"  )); dest+=4; break;
        case '>' : _tcscpy_s(dest,sizeof(dest),_T("&gt;"  )); dest+=4; break;
        case '&' : _tcscpy_s(dest,sizeof(dest),_T("&amp;" )); dest+=5; break;
        case '\'': _tcscpy_s(dest,sizeof(dest),_T("&apos;")); dest+=6; break;
        case '"' : _tcscpy_s(dest,sizeof(dest),_T("&quot;")); dest+=6; break;
...
#define LENSTR(lpsz) (lpsz ? _tcslen(lpsz) : 0)
    ...
    if (cbElement)
    {
        ...
        if (lpszMarker)
        {
            ...
            _tcscpy_s(&lpszMarker[nResult],sizeof(&lpszMarker[nResult]),pEntry->lpszName);
            nResult+=cbElement;
            lpszMarker[nResult++]=_T(' ');
        ...
        for (i=0; i<nIndex; i++)
        {
            ...
            if (cb)
            {
                if (lpszMarker) _tcscpy_s(&lpszMarker[nResult],sizeof(&lpszMarker[nResult]),pAttr->lpszName);
                nResult += cb;
                // "Attrib=Value "
        ...
        case eNodeClear:
            ...
            if (cb)
            {
                if (nFormat!=-1)
                {
                    ...
                    {
                        charmemset(&lpszMarker[nResult], INDENTCHAR, sizeof(TCHAR)*(nFormat + 1));
                        _tcscpy_s(&lpszMarker[nResult+nFormat+1], sizeof(&lpszMarker[nResult+nFormat+1]), ((XMLClear*)pChild)->lpszOpenTag);
                    }
                    nResult+=cb+nFormat+1;
                }
                else
                {
                    if (lpszMarker)_tcscpy_s(&lpszMarker[nResult], sizeof(&lpszMarker[nResult]), ((XMLClear*)pChild)->lpszOpenTag);
                    nResult += cb;
                }
            }
            ...
            if (cb)
            {
                if (lpszMarker) _tcscpy_s(&lpszMarker[nResult], sizeof(&lpszMarker[nResult]), ((XMLClear*)pChild)->lpszValue);
                nResult += cb;
            }
            ...
            if (cb)
            {
                if (lpszMarker) _tcscpy_s(&lpszMarker[nResult], sizeof(&lpszMarker[nResult]), ((XMLClear*)pChild)->lpszCloseTag);
                nResult += cb;
            }
    ...
    if ((cbElement)&&(!pEntry->isDeclaration))
    {
        ...
        if (bHasChildren)
        {
            ...
            if (lpszMarker)
            {
                ...
                _tcscpy_s(&lpszMarker[nResult],sizeof(&lpszMarker[nResult]), _T("</"));
                nResult += 2;
                _tcscpy_s(&lpszMarker[nResult], sizeof(&lpszMarker[nResult]), pEntry->lpszName);
                nResult += cbElement;
